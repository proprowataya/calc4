    ...
    public:
        ...
        virtual void Visit(const DecimalOperator &op) override {
            ...
            if (TryGetPrecomputedValue(precomputed, &precomputedValue)) {
                value = std::make_shared<PrecomputedOperator>(static_cast<TNumber>(precomputedValue * 10 + op.GetValue()));
            } else {
...
template std::shared_ptr<Operator> Optimize<int64_t>(CompilationContext &context, const std::shared_ptr<Operator> &op);
template std::shared_ptr<Operator> Optimize<__int128_t>(CompilationContext &context, const std::shared_ptr<Operator> &op);
template std::shared_ptr<Operator> Optimize<mpz_class>(CompilationContext &context, const std::shared_ptr<Operator> &op);
